    ...
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            <div class="flex items-center justify-center">
                <img src="{{ asset('profile.png') }}" alt="Edit User" class="w-10 h-10 object-contain">
            </div>
            <div>
                <h2 class="font-bold text-2xl text-slate-800 dark:text-slate-200 leading-tight">
                    {{ __('Edit User') }}
                </h2>
                <p class="text-sm text-slate-500 dark:text-slate-400">Edit informasi user: {{ $user->name }}</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('settings.update', $user) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="role" name="role" class="border-slate-300 dark:border-slate-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100" required>
                                ...
                                @endphp
                                <option value="anggota" {{ old('role', $userRole) === 'anggota' ? 'selected' : '' }}>Anggota</option>
                                <option value="bendahara" {{ old('role', $userRole) === 'bendahara' ? 'selected' : '' }}>Bendahara</option>
                            </select>
                        ...
                        <div class="flex items-center justify-end mt-4 space-x-3">
                            ...
                            <button type="submit" class="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-400 to-indigo-400 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-3 px-6 rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105">
                                <img src="{{ asset('flash-sale.png') }}" alt="Update User" class="w-5 h-5 object-contain">
