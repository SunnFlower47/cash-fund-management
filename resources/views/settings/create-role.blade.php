    ...
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            <div class="flex items-center justify-center">
                <img src="{{ asset('flash-sale.png') }}" alt="Tambah Role" class="w-10 h-10 object-contain">
            </div>
            <div>
                <h2 class="font-bold text-2xl text-slate-800 dark:text-slate-200 leading-tight">
                    {{ __('Tambah Role Baru') }}
                </h2>
                <p class="text-sm text-slate-500 dark:text-slate-400">Buat role baru dengan permission yang sesuai</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm overflow-hidden shadow-lg rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                <div class="p-4 text-slate-900 dark:text-slate-100">
                    <form method="POST" action="{{ route('settings.store-role') }}" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="mt-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                <label class="flex items-center space-x-2 p-3 border border-slate-200 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="rounded border-slate-300 dark:border-slate-600 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        ...
                        <div class="flex items-center justify-end mt-4 space-x-3">
                            ...
                            <button type="submit" class="inline-flex items-center space-x-2 bg-gradient-to-r from-green-400 to-blue-400 hover:from-green-500 hover:to-blue-500 text-white font-bold py-3 px-6 rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105">
                                <img src="{{ asset('flash-sale.png') }}" alt="Create Role" class="w-5 h-5 object-contain">
                                <span>Buat Role</span>
                            </button>
                        </div>
                    </form>
